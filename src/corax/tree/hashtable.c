...
bitv_hashtable_t *hash_init(unsigned int n, unsigned int bit_count)
{
  ...
  h->table =
      (bitv_hash_entry_t **)calloc(table_size, sizeof(bitv_hash_entry_t *));
...
void bitv_normalize(corax_split_t bitv, unsigned int bit_count)
{
  ...
  if (!normalized)
  {
    for (i = 0; i < split_len; ++i) { bitv[i] = ~bitv[i]; }
...
string_hashtable_t *string_hash_init(unsigned int n, unsigned int max_labels)
{
  ...
  h->table =
      (string_hash_entry_t **)calloc(table_size, sizeof(string_hash_entry_t *));
...
int string_hash_insert(const char *s, string_hashtable_t *h, int node_number)
{

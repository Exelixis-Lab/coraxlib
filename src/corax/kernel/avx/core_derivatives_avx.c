    ...
    unsigned int        attrib)
{
  ...
  for (n = 0; n < parent_sites; n++)
  {
    for (i = 0; i < rate_cats; ++i)
    {
      ...
      __m256d       v_lefterm[4];
      v_lefterm[0] = v_lefterm[1] = v_lefterm[2] = v_lefterm[3] =
          _mm256_setzero_pd();
      __m256d v_eigen;
      __m256d v_clvp;
      ...
      v_lefterm[0] =
          _mm256_add_pd(v_lefterm[0], _mm256_mul_pd(v_eigen, v_clvp));
      ct_inv_eigenvecs += 4;

      v_eigen = _mm256_load_pd(ct_inv_eigenvecs);
      v_lefterm[1] =
          _mm256_add_pd(v_lefterm[1], _mm256_mul_pd(v_eigen, v_clvp));

      ct_inv_eigenvecs += 4;
      v_eigen = _mm256_load_pd(ct_inv_eigenvecs);
      v_lefterm[2] =
          _mm256_add_pd(v_lefterm[2], _mm256_mul_pd(v_eigen, v_clvp));

      ct_inv_eigenvecs += 4;

      v_eigen = _mm256_load_pd(ct_inv_eigenvecs);
      v_lefterm[3] =
          _mm256_add_pd(v_lefterm[3], _mm256_mul_pd(v_eigen, v_clvp));
  ...
  for (n = 0; n < sites; n++)
  {
    ...
    for (i = 0; i < rate_cats; ++i)
    {
      ...
      __m256d v_righterm[4];
      v_righterm[0] = v_righterm[1] = v_righterm[2] = v_righterm[3] =
          _mm256_setzero_pd();
      ...
      v_eigen = _mm256_load_pd(c_eigenvecs);
      v_righterm[0] =
          _mm256_add_pd(v_righterm[0], _mm256_mul_pd(v_eigen, v_clvc));
      c_eigenvecs += 4;

      v_eigen = _mm256_load_pd(c_eigenvecs);
      v_righterm[1] =
          _mm256_add_pd(v_righterm[1], _mm256_mul_pd(v_eigen, v_clvc));
      c_eigenvecs += 4;

      v_eigen = _mm256_load_pd(c_eigenvecs);
      v_righterm[2] =
          _mm256_add_pd(v_righterm[2], _mm256_mul_pd(v_eigen, v_clvc));
      c_eigenvecs += 4;

      v_eigen = _mm256_load_pd(c_eigenvecs);
      v_righterm[3] =
          _mm256_add_pd(v_righterm[3], _mm256_mul_pd(v_eigen, v_clvc));
      c_eigenvecs += 4;
    ...
    for (j = 0; j < states; ++j) {
      for (k = 0; k < states; ++k)
      {
        tt_inv_eigenvecs[i * states_padded * states_padded + j * states_padded
                         + k] =
            inv_eigenvecs[i][k * states_padded + j] * t_freqs[k];
    ...
    for (i = 0; i < rate_cats; ++i)
    {
      for (j = 0; j < states_padded; j += 4)
      {
        ...
        for (k = 0; k < states_padded; k += 4)
        {
          ...
          v_lefterm0 =
              _mm256_add_pd(v_lefterm0, _mm256_mul_pd(v_eigen, v_clvp));

          v_eigen = _mm256_load_pd(em0 + k);
          v_righterm0 =
              _mm256_add_pd(v_righterm0, _mm256_mul_pd(v_eigen, v_clvc));

          /* row 1 */
          v_eigen = _mm256_load_pd(im1 + k);
          v_lefterm1 =
              _mm256_add_pd(v_lefterm1, _mm256_mul_pd(v_eigen, v_clvp));

          v_eigen = _mm256_load_pd(em1 + k);
          v_righterm1 =
              _mm256_add_pd(v_righterm1, _mm256_mul_pd(v_eigen, v_clvc));

          /* row 2 */
          v_eigen = _mm256_load_pd(im2 + k);
          v_lefterm2 =
              _mm256_add_pd(v_lefterm2, _mm256_mul_pd(v_eigen, v_clvp));

          v_eigen = _mm256_load_pd(em2 + k);
          v_righterm2 =
              _mm256_add_pd(v_righterm2, _mm256_mul_pd(v_eigen, v_clvc));

          /* row 3 */
          v_eigen = _mm256_load_pd(im3 + k);
          v_lefterm3 =
              _mm256_add_pd(v_lefterm3, _mm256_mul_pd(v_eigen, v_clvp));

          v_eigen = _mm256_load_pd(em3 + k);
          v_righterm3 =
              _mm256_add_pd(v_righterm3, _mm256_mul_pd(v_eigen, v_clvc));
        }
                                           ...
                                           unsigned int         attrib)
{
  ...
  const double *t_eigenvecs_trans = NULL;
    ...
    for (i = 0; i < rate_cats; ++i)
    {
      ...
      for (k = 0; k < states; ++k)
      {
        if ((n >> k) & 1)
        {
          ...
          v_lefterm = _mm256_add_pd(v_lefterm, _mm256_mul_pd(v_eigen, v_freqs));
        }
      }
    ...
    for (i = 0; i < rate_cats; ++i)
    {
      for (j = 0; j < states_padded; j += 4)
      {
        ...
        if (ss != -1)
        {
          ...
          __m256d v_eigen =
              _mm256_load_pd(inv_eigenvecs[i] + ss * states_padded + j);
        ...
        else
        {
          ...
          for (k = 0; k < states; ++k)
          {
            if ((state >> k) & 1)
            {
              __m256d v_freqs = _mm256_set1_pd(freqs[i][k]);
              __m256d v_eigen =
                  _mm256_load_pd(inv_eigenvecs[i] + k * states_padded + j);

              v_lefterm =
                  _mm256_add_pd(v_lefterm, _mm256_mul_pd(v_eigen, v_freqs));
            }
          }
        }
  ...
  for (n = 0; n < ef_sites; ++n)
  {
    ...
    for (i = 0; i < rate_cats; ++i)
    {
      ...
      if (states == 4)
      {
        ...
        v_cat_sitelk =
            _mm256_add_pd(v_cat_sitelk, _mm256_mul_pd(v_sum, v_diagp));
      ...
      else
      {
        ...
        v_sitelk =
            _mm256_add_pd(v_sitelk, _mm256_mul_pd(v_cat_sitelk, v_weight));
      }
    }
    ...
    {
      __m256d v_term0 =
          _mm256_setr_pd(site_lk[0], site_lk[4], site_lk[8], site_lk[12]);
      __m256d v_term1 =
          _mm256_setr_pd(site_lk[1], site_lk[5], site_lk[9], site_lk[13]);
      __m256d v_term2 =
          _mm256_setr_pd(site_lk[2], site_lk[6], site_lk[10], site_lk[14]);

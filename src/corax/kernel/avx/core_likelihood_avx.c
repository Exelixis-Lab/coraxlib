  ...
  for (i = 0; i < sites; ++i)
  {
    ...
    for (j = 0; j < rate_cats; ++j)
    {
      ...
      if (prop_invar > 0)
      {
        ...
        term += rate_weights[j]
                * (term_r * (1 - prop_invar) + inv_site_lk * prop_invar);
      }
      else
      {
                                         ...
                                         unsigned int         attrib)
{
  ...
  int           per_rate_scaling =
      parent_scaler && (attrib & CORAX_ATTRIB_RATE_SCALERS) ? 1 : 0;
  ...
  for (n = 0; n < sites; ++n)
  {
    ...
    if (per_rate_scaling)
    {
      ...
      {
        if (pscaler[i] < site_scalings) site_scalings = pscaler[i];
      }
      ...
      {
        rate_scalings[i] =
            CORAX_MIN(pscaler[i] - site_scalings, CORAX_SCALE_RATE_MAXDIFF);
      }
    }
    else
    {
...
corax_core_edge_loglikelihood_ti_20x20_avx(unsigned int         sites,
                                           ...
                                           const double *const *frequencies,
                                           const double *       rate_weights,
                                           const double int *   pattern_weights,
                                           const double *      invar_proportion,
                                           const int *         invar_indices,
                                           const unsigned int *freqs_indices,
                                           double *            persite_lnl,
                                           ...
                                           unsigned int        attrib)
{
  ...
  /* precompute a lookup table of four values per entry (one for each state),
     for all 16 states (including ambiguities) and for each rate category. */
  double *lookup = corax_aligned_alloc(maxstates * span * sizeof(double),
                                       CORAX_ALIGNMENT_AVX);
  if (!lookup)
  {
    /* TODO: in the highly unlikely event that allocation fails, we should
...
CORAX_EXPORT
double corax_core_edge_loglikelihood_ti_avx(unsigned int         states,
                                            unsigned int         sites,
                                            unsigned int         rate_cats,
                                            const double *       parent_clv,
                                            const unsigned int * parent_scaler,
    ...
    for (i = 0; i < rate_cats; ++i)
    {
      ...
      for (j = 0; j < states_padded; j += 4)
      {
        ...
        for (k = 0; k < states_padded; k += 4)
        {
          ...
          /* set mask */
          mask = _mm256_set_epi64x(((cstate >> (lsb + 3)) & 1) ? ~0 : 0,
                                   ((cstate >> (lsb + 2)) & 1) ? ~0 : 0,
                                   ((cstate >> (lsb + 1)) & 1) ? ~0 : 0,
                                   ((cstate >> (lsb + 0)) & 1) ? ~0 : 0);
...
CORAX_EXPORT
double corax_core_edge_loglikelihood_repeats_generic_avx(
    unsigned int         states,
    unsigned int         sites,
...
CORAX_EXPORT
double corax_core_edge_loglikelihood_ii_avx(unsigned int         states,
                                            unsigned int         sites,
                                            unsigned int         rate_cats,
                                            const double *       parent_clv,
                                            const unsigned int * parent_scaler,
...
CORAX_EXPORT
double corax_core_edge_loglikelihood_repeats_4x4_avx(
    unsigned int         states,
    unsigned int         sites,
    const unsigned int   child_sites,
    unsigned int         rate_cats,
...
CORAX_EXPORT
double corax_core_edge_loglikelihood_repeatsbclv_4x4_avx(
    unsigned int         states,
    unsigned int         sites,
    const unsigned int   child_sites,
    unsigned int         rate_cats,

                                  ...
                                  unsigned int        count)
{
  ...
  expd = (double *)corax_aligned_alloc(4 * sizeof(double), CORAX_ALIGNMENT_AVX);
  ...
  for (i = 0; i < count; ++i)
  {
    ...
    for (n = 0; n < rate_cats; ++n)
    {
      ...
      if (branch_lengths[i] > 0.)
      {
        ...
        xmm1 = _mm256_set_pd(
            expm1(expd[3]), expm1(expd[2]), expm1(expd[1]), expm1(expd[0]));
...
#ifdef DEBUG
      for (j = 0; j < 20; ++j)
        for (k = 0; k < 20; ++k) assert(dbg_pmat[j * 20 + k] >= 0);
#endif
    }
  }

                                ...
                                int (*bin_func)(void *, size_t, size_t, FILE *))
{
  ...
  if (partition->attributes & CORAX_ATTRIB_SITE_REPEATS)
  {
    bin_func(partition->repeats->pernode_ids,
             sizeof(unsigned int),
             partition->clv_buffers + tips,
             bin_file);
    bin_func(partition->repeats->perscale_ids,
             sizeof(unsigned int),
  ...
  if (attributes & CORAX_BIN_ATTRIB_PARTITION_DUMP_CLV)
  {
    ...
    if (partition->attributes & CORAX_ATTRIB_PATTERN_TIP)
    {
      ...
      unsigned int l2_maxstates =
          (unsigned int)ceil(log2(partition->maxstates));
    ...
    if (partition->attributes & CORAX_ATTRIB_SITE_REPEATS)
    {
      ...
      for (i = first_clv_index; i < (partition->clv_buffers + tips); ++i)
      {
        ...
        {
          bin_func(partition->repeats->pernode_site_id[i],
                   sizeof(unsigned int),
                   sites_alloc,
                   bin_file);

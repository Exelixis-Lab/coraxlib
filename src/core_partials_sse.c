...
PLL_EXPORT void pll_core_create_lookup_4x4_sse(unsigned int  rate_cats,
  ...
  for (j = 1; j < maxstates; ++j) {
    /* masks for state j */
    xmm1 = _mm_set_pd(((j >> 1) & 1) ? 1 : 0, ((j >> 0) & 1) ? 1 : 0);
    xmm0 = _mm_cmpgt_pd(xmm1, ymm4);

    xmm2 = _mm_set_pd(((j >> 3) & 1) ? 1 : 0, ((j >> 2) & 1) ? 1 : 0);
    ...
    for (k = 1; k < maxstates; ++k) {
      /* masks for state k */
      xmm3 = _mm_set_pd(((k >> 1) & 1) ? 1 : 0, ((k >> 0) & 1) ? 1 : 0);
      xmm2 = _mm_cmpgt_pd(xmm3, ymm4);

      xmm4 = _mm_set_pd(((k >> 3) & 1) ? 1 : 0, ((k >> 2) & 1) ? 1 : 0);
...
PLL_EXPORT void pll_core_create_lookup_sse(unsigned int       states,
  ...
  if (states == 4) {
    pll_core_create_lookup_4x4_sse(
  ...
  for (j = 0; j < maxstates; ++j) {
    ...
    for (k = 0; k < maxstates; ++k) {
      ...
      for (n = 0; n < rate_cats; ++n) {
        ...
        /* reset pointers to the start of the next p-matrix, as the
...
pll_core_update_partial_ti_4x4_sse(unsigned int         sites,
  ...
  double *lookup =
      pll_aligned_alloc(64 * rate_cats * sizeof(double), PLL_ALIGNMENT_SSE);
  if (!lookup) {
  ...
  for (i = 1; i < 16; ++i) {
    ...
    xmm1 = _mm_set_pd(((i >> 1) & 1) ? 1 : 0, ((i >> 0) & 1) ? 1 : 0);
    xmm0 = _mm_cmpgt_pd(xmm1, ymm3);

    xmm2 = _mm_set_pd(((i >> 3) & 1) ? 1 : 0, ((i >> 2) & 1) ? 1 : 0);

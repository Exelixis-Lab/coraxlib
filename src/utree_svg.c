...
static int utree_height_recursive(pll_unode_t *node) {
  ...
  if (d1->height > d2->height) d->height = d1->height + 1;
  else
...
static void draw_line(
    FILE *fp, double x1, double y1, double x2, double y2, double stroke_width) {
  fprintf(fp,
...
static void utree_set_offset(pll_unode_t *           node,
  ...
  if (parent) data->x += parent_data->x;
  else
...
static void utree_plot(FILE *                  fp,
  ...
  if (parent) {
    ...
    if (!node->next) {
      y = aux->tip_occ * attr->tip_spacing + attr->margin_top
          + attr->legend_spacing;
    ...
    /* horizontal line */
    draw_line(fp, px, y, x, y, attr->stroke_width);
    data->y = y;
...
static void utree_scaler_init(const pll_svg_attrib_t *attr,
  ...
  for (i = 0; i < tree->tip_count; ++i) {
    ...
    while (1) {
      ...
      if (nb_data->height > data->height) node = node->next->back;
...
static void print_header(FILE *                  fp,
  ...
  fprintf(fp,
          "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"%ld\" "
  ...
  fprintf(svg_fp, "<rect x=\"%ld\" y=\"%ld\" width=\"%ld\" fill=\"none\" "
          ...
          svg_height - opt_svg_margintop - legend_spacing -
...
PLL_EXPORT int pll_utree_export_svg(pll_utree_t *           tree,
  ...
  /* backup data */
  void **data_old =
      (void **)malloc((tree->tip_count + tree->inner_count) * sizeof(void *));
  if (!data_old) {
    fclose(fp);
  ...
  if (!utree_set_height(root)) rc = PLL_FAILURE;
  else

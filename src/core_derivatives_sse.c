    ...
    for (i = 0; i < rate_cats; ++i) {
      for (j = 0; j < states; j += 2) {
        ...
        for (k = 0; k < states_padded; k += 2) {
          ...
          v_lefterm0 = _mm_add_pd(v_lefterm0, _mm_mul_pd(v_eigen, v_clvp));

          v_eigen     = _mm_load_pd(em0 + k);
          v_righterm0 = _mm_add_pd(v_righterm0, _mm_mul_pd(v_eigen, v_clvc));

          /* row 1 */
          v_eigen    = _mm_load_pd(im1 + k);
          v_lefterm1 = _mm_add_pd(v_lefterm1, _mm_mul_pd(v_eigen, v_clvp));

          v_eigen     = _mm_load_pd(em1 + k);
          v_righterm1 = _mm_add_pd(v_righterm1, _mm_mul_pd(v_eigen, v_clvc));
        }

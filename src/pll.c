...
static int create_charmap(pll_partition_t *  partition,
  ...
  if (!(partition->charmap =
    ...
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Cannot allocate charmap for tip-tip precomputation.");
    return PLL_FAILURE;
  }
  ...
  if (!(partition->tipmap =
    ...
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Cannot allocate tipmap for tip-tip precomputation.");
    return PLL_FAILURE;
  }
  ...
  if (!partition->tipchars) {
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Cannot allocate space for storing tip characters.");
    return PLL_FAILURE;
  }
...
PLL_EXPORT pll_partition_t *pll_partition_create(unsigned int tips,
  ...
  if (!partition->eigenvecs) {
    ...
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Unable to allocate enough memory for eigenvectors.");
    return PLL_FAILURE;
  }
  for (i = 0; i < partition->rate_matrices; ++i) {
  ...
  if (!partition->eigenvals) {
    ...
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Unable to allocate enough memory for eigenvalues.");
    return PLL_FAILURE;
  }
  for (i = 0; i < partition->rate_matrices; ++i) {
  ...
  if (!partition->frequencies) {
    ...
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Unable to allocate enough memory for frequencies.");
    return PLL_FAILURE;
  }
  for (i = 0; i < partition->rate_matrices; ++i) {
  ...
  if (partition->rate_weights) {
    ...
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Unable to allocate enough memory for rate weights.");
    return PLL_FAILURE;
  }
  ...
  if (!partition->scale_buffer) {
    ...
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Unable to allocate enough memory for scale buffers.");
    return PLL_FAILURE;
  }
...
static int set_tipchars_4x4(pll_partition_t *  partition,
  ...
  for (i = 0; i < partition->sites; ++i) {
    if ((c = map[(int)sequence[i]]) == 0) {
      ...
      snprintf(
          pll_errmsg, 200, "Illegal state code in tip \"%c\"", sequence[i]);
      return PLL_FAILURE;
    }
...
static int set_tipclv(pll_partition_t *  partition,
  ...
  for (i = 0; i < ids; ++i) {
    ...
    if ((c = map[(int)sequence[index]]) == 0) {
      pll_errno = PLL_ERROR_TIPDATA_ILLEGALSTATE;
      snprintf(
          pll_errmsg, 200, "Illegal state code in tip \"%c\"", sequence[index]);
      return PLL_FAILURE;
    }

...
static int
encode(char **sequence, const unsigned char *map, int count, int len) {
  int            i, j;
  unsigned char *p;
  unsigned char  c;
...
static unsigned int *compress_site_patterns(char **            sequence,
  ...
  if (!count) {
    ...
    snprintf(pll_errmsg, 200, "Number of sequences must be greater than 0.");
    return NULL;
  }
  ...
  if (!map) {
    ...
    snprintf(pll_errmsg, 200, "Map is undefined.");
    return NULL;
  }
  ...
  if (map[0]) {
    pll_errno = PLL_ERROR_MSA_MAP_INVALID;
    snprintf(pll_errmsg, 200, "'0' cannot be used as a state.");
    return NULL;
  }
  ...
  if (site_pattern_map) {
    ...
    if (!sort_backmap) {
      ...
      snprintf(pll_errmsg, 200, "Cannot allocate space for sort backmap.");
      return NULL;
    }
  ...
  if (!column) {
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(pll_errmsg, 200, "Cannot allocate space for matrix columns.");
    free(sort_backmap);
    return NULL;
  }
  ...
  if (!memptr) {
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(pll_errmsg, 200, "Cannot allocate space for matrix data.");
    free(sort_backmap);
    free(column);
    return NULL;
  }
  ...
  if (!weight) {
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(
        pll_errmsg, 200, "Cannot allocate space for storing site weights.");
    free(sort_backmap);
    free(column);
    free(memptr);
    return NULL;
  ...
  if (!rnd_state) {
    pll_errno = PLL_ERROR_MEM_ALLOC;
    snprintf(pll_errmsg, 200, "Cannot allocate space for storing RNG state.");
    free(weight);
    free(sort_backmap);
    free(column);
    free(memptr);

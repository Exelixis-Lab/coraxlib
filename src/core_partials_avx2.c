...
void pll_core_update_partial_ti_20x20_avx2(unsigned int         sites,
  ...
  for (j = 0; j < maxstates; ++j) {
    ...
    if (scount == 1) {
      ...
      for (n = 0; n < rate_cats; ++n) {
        for (i = 0; i < states; ++i) {
          double terml = 0;
          for (m = 0; m < states; ++m) { terml += lmat[m]; }
          ptr[i] = terml;
  ...
  for (n = 0; n < sites; ++n) {
    ...
    for (k = 0; k < rate_cats; ++k) {
      ...
      __m256d v_rclv[5];
      for (i = 0; i < 5; ++i) { v_rclv[i] = _mm256_load_pd(right_clv + i * 4); }
...
PLL_EXPORT void
pll_core_update_partial_ii_20x20_avx2(unsigned int        sites,
                                      unsigned int        rate_cats,
                                      double *            parent_clv,
                                      unsigned int *      parent_scaler,
...
PLL_EXPORT void
pll_core_update_partial_repeats_20x20_avx2(unsigned int        parent_sites,

  ...
  for (n = 0; n < sites; ++n) {
    ...
    for (k = 0; k < rate_cats; ++k) {
      ...
      if (scale_mode == 2) {
        ...
        if (rate_scale) {
          for (i = 0; i < states; ++i) parent_clv[i] *= PLL_SCALE_FACTOR;
    ...
    if (site_scale) {
      parent_clv -= span;
      for (i = 0; i < span; ++i) parent_clv[i] *= PLL_SCALE_FACTOR;
      parent_clv += span;
      parent_scaler[n] += 1;
    }
  }
...
#endif
  if (states == 4) {
    pll_core_create_lookup_4x4(rate_cats, lookup, left_matrix, right_matrix);
    return;
  }

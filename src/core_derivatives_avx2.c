...
PLL_EXPORT int
pll_core_update_sumtable_ii_20x20_avx2(unsigned int        sites,
  ...
  if (!tt_eigenvecs || !tt_inv_eigenvecs) {
    if (tt_eigenvecs) pll_aligned_free(tt_eigenvecs);
    if (tt_inv_eigenvecs) pll_aligned_free(tt_inv_eigenvecs);
...
PLL_EXPORT int
pll_core_update_sumtable_repeats_20x20_avx2(unsigned int        sites,
...
PLL_EXPORT int
pll_core_update_sumtable_ti_20x20_avx2(unsigned int         sites,
                                       unsigned int         rate_cats,
  ...
  for (n = 0; n < sites; n++) {
    ...
    for (i = 0; i < rate_cats; ++i) {
      __m256d v_rclv[5];
      for (j = 0; j < 5; ++j) { v_rclv[j] = _mm256_load_pd(t_rclv + j * 4); }
